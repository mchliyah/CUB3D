...
/*   Updated: 2023/01/05 14:50:32 by mchliyah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	horizontal_intersection(t_cub *cub, t_player *player, t_ray *ray)
{
	...
	while (ray->next_horz_x >= 0 && ray->next_horz_x
		<= cub->map.width * TILESIZE && ray->next_horz_y
		>= 0 && ray->next_horz_y <= cub->map.height * TILESIZE)
	{
		i = 0;
		if (ray->is_facing_up)
			i = 1;
...
void	vertical_intersection(t_cub *cub, t_player *player, t_ray *ray)
{
	...
	while (ray->next_vert_x >= 0 && ray->next_vert_x
		<= cub->map.width * TILESIZE && ray->next_vert_y
		>= 0 && ray->next_vert_y <= cub->map.height * TILESIZE)
	{
		i = 0;
		if (!ray->is_facing_right)
		i = 1;
